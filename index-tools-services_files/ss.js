...
/**
* Global flag to indicate whether the page is the Search Results or any other page.
* @type {boolean}
*/
...
*/
function ss_handleKey(e, isSearchResultsPage) {
    ...
    var qField = null;
    if (isSearchResults) {
        txtbox = document.getElementById('q');
    }
    else {
        qField = document.getElementsByName('q')[0];
        txtbox = document.getElementById('searchTextbox');
    }
    
    var qnow = (!ss_qbackup) ? txtbox.value : ss_qbackup;
    var sum = 0;
    ...
    switch (kid) {
        case 40:  // "key down".
            ...
            if (ss_processed(qnow)) {
                ...
                if (sum > 0) {
                    ...
                    if (suggestion.q) {
                        txtbox.value = suggestion.q;
                        if (!isSearchResults) {
                            qField.value = txtbox.value;
                        }
                    } else {
                        txtbox.value = ss_qbackup;
                        if (!isSearchResults) {
                            qField.value = txtbox.value;
                        }
                    }
        ...
        default:
            ...
            if (txtbox.value == ss_qshown) {
                ...
                ss_waiting++;
                if (ss_allow_debug && ss_debug && ss_debug.getDebugMode()) {
                    ss_debug.addWaitDebugLine(txtbox.value, 'queue', ss_wait_millisec);
                }
                ss_key_handling_queue = setTimeout(
                'ss_handleQuery("' + ss_escape(txtbox.value) + '", ' +
...
function ss_show(qry) {
    var currentQry = null;
    if (!isSearchResults) {
        currentQry = document.getElementById('searchTextbox').value;
    }
    else {
        currentQry = document.getElementById('q').value;
    }
    if (currentQry != qry) {
